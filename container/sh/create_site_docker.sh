...
echo "\
Usage: create_site_docker.sh [OPTIONS]
Starts containers cluster based on the supplied options.
    ...
    --mysqlpassword     MySQL password that provides metadata support for Apache Hive.
                        Please refer to create_ HDCluster.sh script.
...
#--zk
ZK_QUORUM="zk-0:2181,zk-1:2181,zk-2:2181"
ZK_CLIENT_PORT=2181
...
function hdfs_config() {
    ...
    do
        ...
        nnrpcadresstmp="$NAMENODE_APP-$((i-1)):$NAMENODE_RPC_PORT"
        echo "    <property>" >> ${HDFS_SITE_FILE}
    ...
    do
        namenodeidtmp="$NAMENODE_ID$i"
        nnrpcadresstmp="$NAMENODE_APP-$((i-1)):$NAMENODE_HTTP_PORT"
        echo "    <property>" >> ${HDFS_SITE_FILE}
    ...
    for (( i=1; i<=$SERVERS; i++ ))
    do
        journaltmp="$JOURNAL_APP-$((i-1)):$JOURNAL_SERVER_PORT;${journaltmp}"
    done    
    ...
    echo "      <value>${ZK_QUORUM}</value>" >> ${HDFS_SITE_FILE}
    echo "    </property>" >> ${HDFS_SITE_FILE}
...
function yarn_config() {
    ...
    do        
        ...
        echo "        <value>$YARN_APP-$((i-1))</value>" >> ${YARN_SITE_FILE}
        echo "    </property>" >> ${YARN_SITE_FILE}
    done    

    for (( i=1; i<=$SERVERS; i++ ))
    do        
        webappaddresstmp="$YARN_APP-$((i-1)):${RM_WEBAPP_PORT}"
        echo "    <property>" >> ${YARN_SITE_FILE}
    ...
    echo "        <value>${ZK_QUORUM}</value>" >> ${YARN_SITE_FILE}
    echo "    </property>" >> ${YARN_SITE_FILE}
    echo "    <property>" >> ${YARN_SITE_FILE}
...
function hbase_config() {
    ...
    echo "        <value>${ZK_QUORUM}</value>" >> ${HBASE_SITE_FILE}
    echo "      </property>" >> ${HBASE_SITE_FILE}

    hbktmp=""
    for (( i=1; i<=$SERVERS; i++ ))
    do
        hbktmp="$HBKMASTER_APP-$((i-1)),${hbktmp}"        
    done
    hbktmp="${hbktmp}$HMASTER_APP-0"
...
function mapred_config() {
    ...
    echo "  <value>${JOBHISTORY_APP}-$((i-1)):${JOBHISTORY_PORT}</value>" >> ${MAPRED_SITE_FILE}
    ...
    echo "  <value>${JOBHISTORY_APP}-$((i-1)):${JOBHISTORY_WEBAPP_PORT}</value>" >> ${MAPRED_SITE_FILE}
...
function hive_conf() {
    ...
    echo "            <value>${ZK_QUORUM}</value>" >> ${HIVE_SITE_FILE}
    echo "            <description>" >> ${HIVE_SITE_FILE}
    ...
    for (( i=1; i<=$SERVERS; i++ ))
    do
        hivetmp="thrift://$HIVE_APP-$((i-1)):$HIVE_SERVER2_PORT,${hivetmp}"
    done    
